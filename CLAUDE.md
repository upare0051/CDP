...
3. **The schema** — this section in `CLAUDE.md` (plus a per-project
...
project-level `CLAUDE.md`, offer a one-time bootstrap as a single-line opt-in:

1. Copy `~/.claude/CLAUDE.md` to the repo root as `CLAUDE.md` so the same
...
2. Create a `wiki/` skeleton at the repo root (sibling of any existing
   ...
   short "Wiki for this project" subsection to the new `CLAUDE.md`
...
- Frame as a single sentence ("Want me to bootstrap a project `CLAUDE.md` and
...
- If a project-level `CLAUDE.md` already exists, do nothing — assume it is
  intentional.
