...
def fetch_sync_jobs():
    """Fetch all active sync jobs from Alo ActivationOS API."""
    try:
...
def trigger_sync_job(job_id: int, force_full_refresh: bool = False, **context):
    """
    Trigger a sync job execution via Alo ActivationOS API.
    """
    ...
    response = requests.post(
        f"{ACTIVATIONOS_API_URL}/syncs/{job_id}/trigger",
        headers=get_api_headers(),
...
def create_sync_dag(
    ...
    """
    dag_id = f"activationos_sync_{job_id}"

    dag = DAG(
        dag_id=dag_id,
        default_args=default_args,
        description=description or f"Alo ActivationOS sync job: {job_name}",
        schedule_interval=schedule,
        start_date=days_ago(1),
        catchup=False,
        max_active_runs=1,
        tags=["activationos", "reverse-etl", "sync"],
    )
    ...
    with dag:
        sync_task = PythonOperator(
            ...
            op_kwargs={"job_id": job_id, "force_full_refresh": False},
...
# Dynamic DAG generation from Alo ActivationOS API
try:
...
example_dag = DAG(
    dag_id="activationos_example_sync",
    default_args=default_args,
    description="Example Alo ActivationOS sync DAG for testing",
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["activationos", "example"],
)
